...
# Model 3
beta<-matrix(rnorm(p*f,0,1),p,f); diagc=diag(rep(sqrt(1*f),p))      # error bound = 1
...
for (r in 1:R) {
  ...
  nge <- nge + (permuted_eigenvalues >= obs_eigenvalues) # Increase the count if i-th permuted eigenvalue >= observed i-th eigenvalue
}
# Finally
...
result_model3_gaussian_p1000 <- cbind(obs_eigenvalues, t(permutation_quantiles), p_values)
rownames(result_model3_gaussian_p1000) <- c(seq(1,p,1))
result_model3_gaussian_p1000

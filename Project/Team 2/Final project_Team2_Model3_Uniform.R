...
# Uniform: Data generation of factor model(y=Bf+e)
fm<-matrix(runif(f*n,0,2),f,n)*sqrt(3); epsilon<-matrix(runif(n*p,0,2),p,n)*sqrt(3)
...
result_model3_uniform_p50 <- cbind(obs_eigenvalues, t(permutation_quantiles), p_values)
rownames(result_model3_uniform_p50) <- c(seq(1,p,1))
result_model3_uniform_p50
